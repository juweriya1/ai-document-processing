...
#
# Synchronous PipelineOrchestrator that drove the legacy EasyOCR +
# EntityExtractor path. Superseded by: (a) DocumentProcessor in
# pipeline/document_processor.py for the async state orchestrator, and (b)
# the compiled LangGraph in src/backend/agents/graph.py for the agentic
# pipeline exposed at POST /api/agentic/{id}/process.
#
...
#     No heavy AI logic lives here.
#     Just coordinates modules.
#     """
#
...
#             raise ValueError(f"Invalid state: {doc.status}")
#
...
#
#             # 4. store results (DB layer only)
#             fields = store_extracted_fields(self.db, document_id, extracted.fields)
#             items = store_line_items(self.db, document_id, extracted.line_items)
#
#             # 5. validation (rules layer)
...
#             update_document_status(self.db, document_id, "failed")
#             self.db.commit()
#             raise
#
